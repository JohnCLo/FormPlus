$(function(){
    ...
    var formUrl = 'https://docs.google.com/spreadsheet/formResponse?formkey=dEZ2QVFYRlJfejliZnpJT0p5Y1gwWUE6MQ';
    var formUrl_restaurant = 'https://docs.google.com/spreadsheet/formResponse?formkey=dEdHbWNPQlhpSnAzSnFoclBRMFkzdnc6MQ';

    // Set up map
    var m = mapbox.map('map').addLayer(mapbox.layer().id('mapbox.mapbox-streets'));
    ...
    mapbox.converters.googledocs('0AuT3j4L_PY-vdEdvUjRNWmpySy0xOXprRFNsbFZFaHc', 'od4', typeAhead);

    //Set up date pickers
    var now = new Date();
    now = now.getMonth() + '/' + now.getDate() + '/' + now.getFullYear();
    $('#entry_8').val(now).datepicker();
